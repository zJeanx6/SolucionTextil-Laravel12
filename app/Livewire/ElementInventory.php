...
class ElementInventory extends Component
{
    ...
    {
        $this->elementTypes = ElementType::where('company_nit', Auth::user()->company_nit)->orderBy('name')->get();
    ...
    private function hasRelatedData(Element $element)
    {
        return DB::table('loan_details')
            ->where('element_code', $element->code)
            ->exists() || DB::table('shopping_details')
            ->where('element_code', $element->code)
            ->exists();
    }
    ...
    public function deleteConfirmed($code)
    {
        ...
        if ($this->hasRelatedData($element)) {
            $this->dispatch('event-notify', 'No se puede eliminar el elemento porque está relacionado con movimientos de préstamos o compras.');
            return;
        }

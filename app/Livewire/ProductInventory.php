...
class ProductInventory extends Component
{
    ...
    public $code, $name, $stock, $photo, $image_path, $company_nit;
    ...
    {
        $this->productTypes = ProductType::where('company_nit', Auth::user()->company_nit)->orderBy('name')->get();
    ...
    private function hasRelatedData(Product $product)
    {
        $hasTicketDetails = DB::table('ticket_details')
            ->where('product_code', $product->code)
            ->exists();

        $hasExitDetails = DB::table('exit_details')
            ->where('product_code', $product->code)
            ->exists();

        return $hasTicketDetails || $hasExitDetails;
    }
    ...
    public function deleteConfirmed($code)
    {
        ...
        if ($this->hasRelatedData($product)) {
            $this->dispatch('event-notify', 'No se puede eliminar el producto porque está relacionado con otros registros.');
            return;
        }
        ...
        $this->dispatch('event-notify', 'Producto eliminado exitosamente.');
    }
    ...
    public function render()
    {
        $products = Product::query()
            ...
            // Filtra por el company_nit del usuario autenticado
